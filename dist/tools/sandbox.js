...
const LICENSE_TYPES = ["DEVELOPER", "DEVELOPER_PRO", "PARTIAL", "FULL"];
...
export function registerSandboxTools(server) {
    // ---------------- salesforce_list_sandboxes ----------------
    server.registerTool("salesforce_list_sandboxes", {
...
Note: This tool must be run against the PRODUCTION org (sandboxes live under prod).`,
        ...
        try {
            ...
            const info = await sf.toolingQuery("SELECT Id, SandboxName, LicenseType, Description FROM SandboxInfo ORDER BY SandboxName");
            const processes = include_in_progress
                ? await sf
                    .toolingQuery("SELECT Id, SandboxName, Status, CopyProgress, LicenseType, Description, StartDate, EndDate FROM SandboxProcess ORDER BY CreatedDate DESC LIMIT 20")
                    .catch(() => ({ totalSize: 0, done: true, records: [] }))
                : null;
            const output = {
            ...
            for (const rec of info.records) {
                lines.push(`| \`${String(rec.SandboxName ?? "-")}\` | ${String(rec.LicenseType ?? "-")} | ${String(rec.Description ?? "")} |`);
            }
            if (processes && processes.records.length) {
                ...
                for (const rec of processes.records) {
                    lines.push(`| \`${String(rec.SandboxName ?? "-")}\` | ${String(rec.Status ?? "-")} | ${String(rec.CopyProgress ?? 0)}% | ${String(rec.LicenseType ?? "-")} | ${String(rec.StartDate ?? "-")} | ${String(rec.EndDate ?? "-")} |`);
                }
            }
            return {
    ...
    server.registerTool("salesforce_create_sandbox", {
