...
class SalesforceClient {
    ...
    authenticateSfCli() {
        ...
        try {
            ...
            const displayOut = execSync(`sf org display --target-org ${username} --json 2>/dev/null`, { encoding: "utf-8" });
            const displayJsonStart = displayOut.indexOf("{");
            const displayData = JSON.parse(displayJsonStart >= 0 ? displayOut.slice(displayJsonStart) : displayOut);
            const instanceUrl = displayData.result?.instanceUrl;
            let accessToken = displayData.result?.accessToken;
            if (!instanceUrl) {
                throw new Error(`SF CLI returned no instanceUrl for ${username}. ` +
            ...
            const isRedacted = !accessToken ||
                (typeof accessToken === "string" && accessToken.toUpperCase().includes("REDACTED"));
            if (isRedacted) {
                try {
                    const tokenOut = execSync(`sf org auth show-access-token --target-org ${username} --json 2>/dev/null`, { encoding: "utf-8" }).trim();
                    ...
                    throw new Error(`SF CLI redacts accessToken and 'sf org auth show-access-token' failed for ${username}. ` +
            ...
            if (!accessToken || String(accessToken).toUpperCase().includes("REDACTED")) {
                throw new Error(`SF CLI returned no usable accessToken for ${username}. ` +
                    "Run 'sf org login web --instance-url <url>' to re-authenticate.");
            }
