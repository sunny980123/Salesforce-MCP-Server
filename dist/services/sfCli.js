 ...
 */
export function resolveMetadataPaths(type, apiName, objectName) {
    switch (type) {
        case "Flow":
        ...
        case "ValidationRule":
            if (!objectName) {
                throw new Error("object_name is required for ValidationRule (parent sObject API name).");
            }
            return {
 ...
 */
export function createTempSfdxProject() {
    ...
    writeFileSync(join(dir, "sfdx-project.json"), JSON.stringify(sfdxProject, null, 2));
 ...
 */
export function writeMetadataFiles(projectDir, paths, xmlContent, bodyContent) {
    ...
    if (paths.bodyPath) {
        if (bodyContent === undefined) {
            throw new Error(`body_content is required for this metadata type (expected ${paths.bodyPath}).`);
        }
 ...
 */
export function readRetrievedFiles(projectDir, paths) {
    const base = join(projectDir, "force-app", "main", "default");
    const xmlFull = join(base, paths.xmlPath);
    if (!existsSync(xmlFull)) {
        throw new Error(`Retrieved file not found at ${paths.xmlPath}. The component may not exist in the org, or retrieval returned nothing.`);
    }
