...
function requireTargetOrg(): string {
  ...
  if (!target) {
    throw new Error(
      "Deploy/retrieve tools require SALESFORCE_SF_CLI_USERNAME (SF CLI auth). " +
...
const metadataTypeEnum = z.enum(
  SUPPORTED_METADATA_TYPES as unknown as [MetadataType, ...MetadataType[]]
);
  ...
  Deploy status with per-component successes and failures (with error messages).`,
    ...
    async ({
      ...
      try {
        ...
        const result = runSfJson(args, { cwd: projectDir }) as {
          status?: number;
          result?: {
            status?: string;
            success?: boolean;
            details?: {
              componentSuccesses?: Array<Record<string, unknown>>;
              componentFailures?: Array<Record<string, unknown>>;
            };
            deployedSource?: Array<Record<string, unknown>>;
        ...
        const successes =
          details?.componentSuccesses ?? result.result?.deployedSource ?? [];
