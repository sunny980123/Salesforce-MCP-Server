      ...
      try {
        ...
        if (response_format === ResponseFormat.JSON) {
          ...
          for (const f of fields) {
            lines.push(
              `| ${f.name} | ${f.label} | ${f.type} | ${f.required ? "✓" : ""} | ${f.createable ? "✓" : ""} | ${f.updateable ? "✓" : ""} |`
            );
            if (include_picklists && f.picklistValues?.length) {
              lines.push(
                `|   | _Values: ${(f.picklistValues as string[]).join(", ")}_ |  |  |  |  |`
              );
            }
          }
          text = lines.join("\n");
      ...
      try {
        ...
        if (response_format === ResponseFormat.JSON) {
          ...
          for (const o of objects) {
            lines.push(
              `| ${o.name} | ${o.label} | ${o.queryable ? "✓" : ""} | ${o.createable ? "✓" : ""} | ${o.updateable ? "✓" : ""} | ${o.deletable ? "✓" : ""} |`
            );
          }
          if (output.has_more) {
